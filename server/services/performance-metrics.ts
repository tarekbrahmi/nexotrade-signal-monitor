...
export class PerformanceCalculator {
   ...
   */
  calculateMetrics(signal: TradeSignal, currentPrice: number): PerformanceMetrics {
    ...
    const riskRewardRatio = this.calculateRiskRewardRatio(entryPrice, targetPrice, stopLossPrice);
    
    // Signal Strength (1-5 rating based on risk-reward and leverage)
    const signalStrength = this.calculateSignalStrength(riskRewardRatio, leverage);
  ...
  private calculateSignalStrength(riskRewardRatio: number, leverage: number): number {
  ...
  private determineMarketTrend(signalType: string): 'bullish' | 'bearish' | 'neutral' {
