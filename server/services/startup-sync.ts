...
export class StartupSyncService {
  ...
  async synchronizeSignals(): Promise<void> {
    console.log('🔄 Starting signal synchronization at microservice startup...');
    ...
    try {
      ...
      for (const signal of activeSignals) {
        ...
        if (now > expiryTime) {
          ...
          console.log(`❌ Expired signal ${signal.uuid} (created: ${signal.created_at}, ttl: ${signal.ttl})`);
          expiredSignals++;
          continue;
        }
        ...
        if (!existingSignal) {
          ...
          const remainingTtlSeconds = Math.floor((expiryTime.getTime() - now.getTime()) / 1000);
          ...
          if (remainingTtlSeconds > 0) {
            ...
            console.log(`✅ Added missing signal ${signal.uuid} to Redis (TTL: ${Math.floor(remainingTtlSeconds/3600)}h remaining)`);
            addedToRedis++;
          }
        }
      }

      console.log(`✓ Synchronization complete: ${addedToRedis} signals added to Redis, ${expiredSignals} signals expired`);
