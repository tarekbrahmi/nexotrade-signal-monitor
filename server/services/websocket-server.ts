...
import { TraderConnection, traderConnectionSchema, SignalUpdateMessage } from '@shared/schema';
...
export class WebSocketServer {
  ...
  constructor(httpServer: HTTPServer, private signalMonitor: SignalMonitor) {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('authenticate', async (data) => {
        try {
          ...
          if (decoded) {
            ...
            console.log(`Trader authenticated for channel ${connectionData.channel_id}`);
          } else {
            socket.emit('authentication_failed', { error: 'Invalid JWT token' });
            socket.disconnect();
          }
        } catch (error) {
      ...
      socket.on('disconnect', () => {
        ...
        if (trader) {
          ...
          const channelConnections = this.channelConnections.get(trader.channelId);
